...
class RespiratoryMeasure(BaseModel):
    ...
    bpm: int
    temperature: float  
    # Ajout optionnel pour le GPS envoyé par Flutter
...
async def analyze(measure: RespiratoryMeasure):
    ...
    if ai_engine is None: 
        raise HTTPException(status_code=503, detail="IA non prête")
    ...
    risk_score = ai_res.get('risk_score', 0.0)
...
async def update_profile(patient_id: str, profile: ProfileUpdate):
    try:
        ...
        if profile.photo_base64 is not None:
            updates.append("photo_base64 = %s"); params.append(profile.photo_base64)

        if not updates: return {"status": "no update needed"}
...
async def get_status(patient_id: str):
    ...
    cur.execute("""
        ...
        FROM sensor_data WHERE patient_id = %s 
        ORDER BY timestamp DESC LIMIT 1
    """, (patient_id,))
    res = cur.fetchone(); cur.close(); conn.close()
    if res:
        mobile_config = generate_mobile_response(res[0], res[1], res[2])
        return {
            "data_id": res[7], "status": res[0], "recommendation": res[1],
            "spo2": float(res[2]), "bpm": int(res[3]), "risk_score": float(res[4]),
            "temperature": float(res[5]), "color": mobile_config["color"],
            "emergency": mobile_config["emergency"], "last_update": res[6].isoformat()
        }
